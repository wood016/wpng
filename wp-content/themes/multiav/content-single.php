 ...
 */	
	...
	global $post;
	if (theme_get_option('theme_single_artcle_ads') && theme_get_option('theme_show_single_artcle_ads')) {
	$arr_ads = explode("|", trim(theme_get_option('theme_single_artcle_ads')));
	$ads_id = array_rand($arr_ads,1);
	$ADS = '<div class="singleartcleads">' .$arr_ads[$ads_id]. '</div>';
	}
	theme_post_wrapper(
		array(
				...
				'before' => theme_get_metadata_icons('tag,sharecode', 'header'),
				'content' =>  $ADS . theme_get_content() ,
				'after' => theme_get_metadata_icons('category,posts_link,date', 'footer')
		)
	);
?>