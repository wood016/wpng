...
?>
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/jquery.tools.min.js"></script>
<link rel="stylesheet" href="<?php bloginfo('template_url') ?>/library/theme_admin.css" type="text/css" media="screen" />

<div class="wrap">
	<div class="clear"></div>
<div id="adminnav">
<ul class="tabs">
	<?php foreach ($theme_options as $op) {	
		$type = theme_get_array_value($op, 'type');
		$name = theme_get_array_value($op, 'name');
		if ($type == 'heading'){ 
			echo '<li><a href="#" title="'.$name.'">' . $name . '</a></li>';
       } 
    }?>
</ul>
</div>
...
<?php
  ...
  echo '<form method="post" class="clear">'."\n";
  ?>
  <p>
  ...
  <?php
	...
	foreach ($theme_options as $op) {
        ...
        $row_odd = 1;
			...
			$row_odd += 1;
			$odd = ( $row_odd % 2 ) ? ' class="odd" ' : '';
			echo '<tr valign="top" '.$odd.'>'."\n";
...
function theme_print_option_control($op, $val){
	...
	switch ( $type) {
		...
		break;
		case 'textarea':
		echo '<textarea name="'.$id.'" id="'.$id.'" rows="5" cols="50" class="large-text code">'.esc_html($val).'</textarea><br />'."\n";
		break;
		case "radio":
			foreach ($op['options'] as $key=>$option) {
